	...
	foreach ($variable as $var)
	{
	   ...
	   else if ($var{0} == '.')
	   {
		...
		else if (substr($var,0,2) == '->')
		{
			...
			{
				throw new Template_Exception('call to internal object members is not allowed', $object);
			}
		...
		{
			throw new Template_Exception('#' . $var_name.implode('', $variable) . '# is an invalid reference', $object);
		}
	}
	return $_result;
}
